...
class Config:
    ...
    Attributes:
        ...
        face_preview: Exibição da captura: ``"gui"`` (na janela do aplicativo,
            padrão), ``"window"`` (janela avulsa do OpenCV), ``"file"`` (grava
            JPEG em disco — DEPURAÇÃO, viola o RNF04 enquanto ligado),
            ``"ascii"`` (recorte no terminal) ou ``"off"``.
...
def load_config():
    ...
    return Config(
        ...
        face_preview=os.environ.get("SENTINEL_FACE_PREVIEW", "gui").lower(),
